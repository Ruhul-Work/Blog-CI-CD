...
jQuery(document).ready(function () {
    ...
    if (!excludedPaths.some(path => currentPage.includes(path))) {
        if (localStorage.getItem('mode') === 'night') {
            jQuery('body').addClass('night-mode');
        }
    }
...
jQuery(document).on('click', '.dark-mode a', function () {
    jQuery('body').addClass('night-mode');
    localStorage.setItem('mode', 'night'); // Save the mode to localStorage
});
 
// Handle light mode activation
jQuery(document).on('click', '.light-mode a', function () {
    jQuery('body').removeClass('night-mode');
    localStorage.setItem('mode', 'day'); // Save the mode to localStorage
});
