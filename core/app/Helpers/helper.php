...
if (! function_exists('canAccessPremiumBlog')) {

    function canAccessPremiumBlog()
    {
        $user = Auth::user();
        ...
        $subscription = SubscriptionOrder::where('user_id', $user->id)
...
if (! function_exists('banglaDate')) {
    function banglaDate($date)
    {
        ...
        $month     = date('F', $timestamp); // Month in English
        $year      = date('Y', $timestamp); // Year

        // Convert day and year digits to Bangla
