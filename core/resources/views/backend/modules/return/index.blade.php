...
@section('meta')
    <title>All Returns - {{ get_option('title') }}</title>
@endsection
@section('content')
        ...
        <ul class="table-top-head">
            ...
            <li>
                <a data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" class="delete-btn-group"
                   href="{{route("returns.all.delete")}}"><i data-feather="trash-2"
            ...
            <li>
                <a data-bs-toggle="tooltip" data-bs-placement="top" title="Update Payment Status" class="paymentStatusChange  bg-info text-white"
                   href="{{ route('returns.payment.status') }}">Payment Status
                </a>
            </li>
            {{-- <li>
                <a data-bs-toggle="tooltip" data-bs-placement="top" title="Assign Courier" class="assignCourier  bg-success text-white"
                   href="{{ route('purchases.courier') }}">Assign Courier
                </a>
            </li> --}}
        </ul>

        <div class="page-btn d-flex">
            <a href="{{ route('returns.create') }}" class="btn btn-added me-2 ">
                <i data-feather="plus-circle" class="me-1"></i>Add New Returns
            </a>
        </div>
    </div>
            ...
            <div class="table-responsive  product-list">
                <table class="table   AjaxDataTable"
                    ...
                    <tr>
                        ...
                        <th class="no-sort">Return Number</th>
                        <th class="no-sort">Return Date</th>
...
@section('script')
    <script>
        ...
        var AJAX_URL = '{{ route('returns-product.ajax') }}';
...
`,
                        ...
                        if (result.isConfirmed) {
                            ...
                            var selectedIds = get_checked();

                            $.ajax({
                                type: "GET",
                                url: `${href}?payment_status=${selectedStatus}&purchase_ids=${encodeURIComponent(JSON.stringify(selectedIds))}`,
                                success: function(response) {
                ...
                if (checkedRows.length > 0) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            var selectedPurchaseIds = get_checked();

                            $.ajax({
                                type: "GET",
                                url: `${href}?courier_id=${selectedCourierId}&purchase_ids=${encodeURIComponent(JSON.stringify(selectedPurchaseIds))}`,
                                success: function(response) {
                                    Swal.fire("Success!", response.message, "success");
                                    $('.AjaxDataTable').DataTable().ajax.reload();
                                },
