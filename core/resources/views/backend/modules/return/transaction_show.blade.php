...
@section('meta')
    <title>Return Transaction Details - {{ get_option('title') }}</title>
@endsection
@section('content')
        ...
        <ul class="table-top-head">
            <li>
                <div class="page-btn d-flex">
                    ...
                    <a href="{{ route('returns.edit', $return->id) }}" class="btn btn-info"><i data-feather="edit"
                ...
                <div class="card-body">
                    ...
                    <div class="invoice-box table-height"
                        ...
                        <div class="sales-details-items d-flex">
                            ...
                            <div class="details-item">
                                <h5><span>{{ $return->return_number }}</span> {{ $return->payment_status }}<br></h5>
                            </div>
                        </div>
                        <h5 class="order-text">Return Transaction Summary</h5>
                        ...
                        <form action="#" method="POST" id="orderPaymentUpdate" enctype="multipart/form-data">
                            ...
                            <div class="col-md-12 d-flex">
                                <div class="col-md-6 mt-3">
                                    ...
                                    <div class="form-group">
                                        <label for="customerName" class="fw-bold">Net
                                            Payable:{{ formatPrice($return->total) }}</label>
                                        <span class="ml-2" id="net_payable"></span>
                                    </div>
                                    <div class="form-group">
                                        <label for="amount" class="fw-bold">Previous
                                            Payment:
                                            {{ formatPrice($return->transactions->sum('amount') + $return->wallets->sum('amount')) }}</label>
                                        <span class="ml-2" id="previous_payment"></span>
                                    </div>
                                    <div class="form-group">
                                        <label for="pay"
                                            class="fw-bold">Due:{{ formatPrice($return->total - ($return->transactions->sum('amount') + $return->wallets->sum('amount'))) }}</label>
                                ...
                                <div class="col-md-6">
                                    <div class="table-responsive">
                                        <table class="table">
                                            ...
                                            <tbody>
                                                @if ($return->transactions && !$return->transactions->isEmpty())
                                                    @foreach ($return->transactions as $transaction)
                                                        <tr>
                                                            <td>{{ $transaction->paymentMethod->name }}</td>
                                                            <td>{{ \Carbon\Carbon::parse($transaction->created_at)->format('Y/m/d h:i A') }}
                                                            </td>
                                                            <td>{{ formatPrice($transaction->amount) }}</td>
                                                            <td>{{ $transaction->user->name ?? '' }}</td>
                                                            <td class="action-table-data">
                                                                <div class="edit-delete-action">
                                                                    <a class="confirm-text p-2 delete-btn"
                                                                        href="{{ route('returns.transactions.destroy', encrypt($transaction->id)) }}">
                                                ...
                                                @else
                                                    <tr>
                                                        <td colspan="5">No previous payment available</td>
                                                    </tr>
                                                @endif

                                                @if ($return->wallets && !$return->wallets->isEmpty())
                                                    @foreach ($return->wallets as $refund)
                                                        <tr>
                                                            <td>{{ $refund->payment_method }}</td>
                                                            <td>{{ \Carbon\Carbon::parse($refund->created_at)->format('Y/m/d h:i A') }}
                                                            </td>
                                                            ...
                                                            <td class="action-table-data">
                                                                <div class="edit-delete-action">
                                                                    <a class="confirm-text p-2 delete-btn"
                                                                        href="{{ route('returns.transactions.destroy', encrypt($refund->id)) }}">
                                                                        <i data-feather="trash-2"
                                                                            class="feather-trash-2"></i>
                                                                    </a>
                                                                </div>
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <th class="fw-bold">
                                                        {{ formatPrice($return->transactions->sum('amount') + $return->wallets->sum('amount')) }}
                                                    </th>
                                                    <th colspan="2"></th>
                                                </tr>
                                            </tfoot>
                                ...
                                <div class="block-section payment-method">
                                    ...
                                    <div class="row d-flex align-items-center justify-content-center methods">
                                        @foreach ($paymentMethods as $paymentMethod)
                                            <div class="col-md-4 col-lg-3 paymentMethodSelect mb-3" data-bs-toggle="modal"
                                                data-bs-target="#payment" data-payment-id="{{ $paymentMethod->id }}">
                                                <div class="default-cover">
                                                    <a href="javascript:void(0);">
                                                        <img src="{{ image('theme/admin/assets/img/icons/cash-pay.svg') }}"
                                                            alt="Payment Method">
                                        ...
                                        <div class="col-md-4 col-lg-3 paymentMethodSelect mb-3" data-bs-toggle="modal"
                                            data-bs-target="#wallet" data-payment-id="">
                                            <div class="default-cover">
                                                <a href="javascript:void(0);">
                                                    <img src="{{ image('theme/admin/assets/img/icons/cash-pay.svg') }}"
                                                        alt="Payment Method">
                            ...
                            <div class="text-center col-12 mb-2">
                                <button type="submit" class="btn btn-success fw-bolder py-3 shadow-lg">Save
                                    Payment</button>
                            </div>
                        </form>
                    </div>
    ...
    <div class="modal fade" id="wallet" tabindex="-1" aria-labelledby="create" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form id="wallettMethod" action=" " method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
                            <div class="row">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#orderPaymentMethod').on('submit', function(event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('returns.method.store') }}',
                    data: form_data,
                    success: function(response) {
                        Swal.fire({
                            icon: 'success',
            ...
            $('#orderPaymentUpdate').on('submit', function(event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('returns.transactions.store', $return->id) }}',
                    data: form_data,
                    success: function(response) {
                        ...
                        if (response.message) {
                            Swal.fire({
                                ...
                                window.location = "{{ route('returns.index') }}";
                            });
                        }
                    },
                    error: function(xhr, status, error) {
                            ...
                            error: function(xhr, status, error) {
                                ...
                                if (xhr.responseJSON && xhr.responseJSON.error) {
                                    ...
                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Error',
                                        text: xhr.responseJSON.errors ? Object
                                            .values(xhr.responseJSON.errors)[0][
                                                0
                                            ] :
                                            'An error occurred while submitting the form. Please try again.'
                                    });
        ...
        $(document).ready(function() {
            $('#wallettMethod').on('submit', function(event) {
                event.preventDefault();
                var form_data = $(this).serialize();
                $.ajax({
                    type: 'POST',
                    url: '{{ route('returns.refund') }}',
                    data: form_data,
                    success: function(response) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: response
                                .message // Adjusted to match the response from your controller
                        });
                        $('.summary').html(response.html);

                        $('#wallet').modal('toggle'); // Changed modal ID to #wallet
                    },
                    error: function(xhr, status, error) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: xhr.responseJSON && xhr.responseJSON.errors ?
