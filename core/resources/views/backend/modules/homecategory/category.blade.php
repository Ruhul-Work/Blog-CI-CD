...
@section('meta')
    <title>All Category - {{ get_option('title') }}</title>
@endsection
        ...
        <div class="card-body p-2">
            <div class="row">
                <div class="mb-3 text-center">
                    <a href="{{ route('home-category.add', ['id' => $hCategory->id]) }}" data-bs-toggle="tooltip"
                        data-bs-placement="top" title="Add To Category"><button type="button"
                            class="btn btn-success">Add
                            into Category</button></a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card table-list-card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table categoryAjaxDataTable  table-hover" style="width:100%;" id="items">
                            <thead>
                                <tr>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            var table;
            if ($('.categoryAjaxDataTable').length > 0) {
                table = $('.categoryAjaxDataTable').DataTable({
                    "bFilter": true,
                    "sDom": 'fBtlpi',
                    "ordering": true,
                    "responsive": true,
                    ...
                    'serverMethod': 'post',
                    'ajax': {
                        url: "{{ route('home-category.ajax', ['id' => $hCategory->id]) }}"
                    },
                    'aLengthMenu': [
                        [10, 50, 100, 200, 500, -1],
                    ...
                    initComplete: function(settings, json) {
                        ...
                        table.on('row-reorder', function(e, details, edit) {
                            ...
                            var tableId = $('.categoryAjaxDataTable').attr('id');

                            $.ajax({
                                url: "{{ route('home-category.sorting') }}",
                                type: 'POST',
                                data: {
                                    tableId: tableId,
                                    newOrder: newOrder
