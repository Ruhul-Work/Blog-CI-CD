...
@section('meta')
    <title>All Pulishers - {{ get_option('title') }}</title>
@endsection
        ...
        <div class="card-body p-2">
            <div class="row">
                <div class="mb-3 text-center">
                    <a href="{{ route('home-category.publisher-add', ['id' => $hCategory->id]) }}" data-bs-toggle="tooltip"
                        data-bs-placement="top" title="Add To Publisher"><button type="button" class="btn btn-success">Add
                            To Publisher</button></a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card table-list-card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table publisherAjaxDataTable  table-hover" style="width:100%;" id="items">
                            <thead>
                                <tr>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            var table;
            if ($('.publisherAjaxDataTable').length > 0) {
                table = $('.publisherAjaxDataTable').DataTable({
                    "bFilter": true,
                    "sDom": 'fBtlpi',
                    "ordering": true,
                    "responsive": true,
                    ...
                    'serverMethod': 'post',
                    'ajax': {
                        url: "{{ route('home-category.publisher-view-ajax', ['id' => $hCategory->id]) }}"
                    },
                    'aLengthMenu': [
                        [10, 50, 100, 200, 500, -1],
                    ...
                    initComplete: function(settings, json) {
                        ...
                        table.on('row-reorder', function(e, details, edit) {
                            ...
                            var tableId = $('.publisherAjaxDataTable').attr('id');

                            $.ajax({
                                url: "{{ route('home-category.publisher-sorting') }}",
                                type: 'POST',
                                data: {
                                    tableId: tableId,
                                    newOrder: newOrder
