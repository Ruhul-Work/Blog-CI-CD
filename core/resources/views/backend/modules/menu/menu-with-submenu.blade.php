        ...
        <div class="card-body p-2">
            <div class="row">
                <div class="mb-3 text-center">
                    <a href="{{ route('menus.submenu.add', ['id' => encrypt($menu->id)]) }}" data-bs-toggle="tooltip"
                        data-bs-placement="top" title="Add To New SubMenu This Menu"><button type="button"
                            class="btn btn-success">Add
                            New SubMenu This Menu</button></a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card table-list-card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table subMenuDataTable  table-hover" style="width:100%;" id="items">
                            <thead>
                                <tr>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            var table;
            if ($('.subMenuDataTable').length > 0) {
                table = $('.subMenuDataTable').DataTable({
                    "bFilter": true,
                    "sDom": 'fBtlpi',
                    "ordering": true,
                    "responsive": true,
                    ...
                    'serverMethod': 'post',
                    'ajax': {
                        url: "{{ route('menus.submenu.ajax-view', ['id' => $menu->id]) }}", // Laravel route for AJAX data
                    },
                    'aLengthMenu': [
                        [10, 50, 100, 200, 500, -1],
                    ...
                    initComplete: function(settings, json) {
                        ...
                        table.on('row-reorder', function(e, details, edit) {
                            ...
                            var tableId = $('.subMenuDataTable').attr(
                                'id'); // Get table ID if needed
                            $.ajax({
                                url: "{{ route('menu.sub-menu-sorting') }}", // Laravel route for sorting
                                type: 'POST',
                                data: {
                                    tableId: tableId,
                                    newOrder: newOrder
