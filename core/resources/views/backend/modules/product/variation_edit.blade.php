                                        ...
                                        <div class="mb-3 add-product">
                                            ...
                                            <select name="variant_id" id="variant_id" class="select">
                                                @foreach ($variations as $variation)
                                                    <option value="{{ $variation->id }}" {{ $variation->id == $variant->id ? 'selected' : '' }}>{{ $variation->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-6 col-sm-6 col-12">
                                        <div class="mb-3 add-product">
                                            ...
                                            <input type="text" name="price" id="price" class="form-control" value="{{ $variant->pivot->price }}" placeholder="Enter price">
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-sm-6 col-12">
                                        <div class="mb-3 add-product">
                                            <label class="form-label">Stock</label>
                                            <input type="text" name="stock" id="stock" class="form-control" value="{{ $variant->pivot->stock }}" placeholder="Enter stock quantity">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="mb-3 add-product">
                                            ...
                                            <textarea name="description" id="description" class="form-control textarea" placeholder="Enter description">{{ $variant->pivot->description }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#createProduct').submit(function (e) {
                ...
                // Make AJAX request
                $.ajax({
                    url: '{{ route('products.variation.update',['productId'=>$product->id,'variationId'=>$variant->id]) }}',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
