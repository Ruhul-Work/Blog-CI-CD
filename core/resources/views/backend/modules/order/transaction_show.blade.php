            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="invoice-box table-height" style="max-width: 1600px;width:100%;overflow: auto;padding: 0;font-size: 14px;line-height: 24px;color: #555;">
                            <div class="sales-details-items d-flex text-capitalize">
                                ...
                                <div class="details-item">
                                    <h5><span>{{$order->order_number}}</span> {{$order->payment_status }}<br>{{ $order->status->name ?? 'No Status' }}</h5>
                                </div>
                            </div>
                            <hr>
                            <h5 class="order-text">Order Transaction Summary</h5>
...
{{--                                                        <input type="hidden" name="id" value="{{$order->id}}">--}}
                                                        <div class="form-group">
                                                            <label for="customerName" class="fw-bold">Net Payable:{{formatPrice($order->total)}}</label>
                                                            <span class="ml-2" id="net_payable"></span>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="amount" class="fw-bold">Previous Payment:{{formatPrice($order->transactions->sum('amount'))}}</label>
                                                            <span class="ml-2" id="previous_payment"></span>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="pay" class="fw-bold">Pay:{{formatPrice($order->total- $order->transactions->sum('amount'))}}</label>
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="table-responsive">
                                                            <table class="table  ">
                                                                ...
                                                                @if($order->transactions && !$order->transactions->isEmpty())
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td class="action-table-data">
                                                                                <div class="edit-delete-action">
                                                                                    <a class="confirm-text p-2 delete-btn" href="{{ route('orders.transactions.destroy', encrypt($transaction->id)) }}">
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <th class="fw-bold">{{formatPrice($order->transactions->sum('amount'))}}</th>
                                                                    <th colspan="2" > </th>
                                                                </tr>
                                                                </tfoot>
                                                            </table>
                                                ...
                                                <div class="text-center col-12 mb-2">
                                                    <button type="submit" class="btn btn-success fw-bolder py-3 shadow-lg">Save Order Payment</button>
                                                </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('#orderPaymentUpdate').on('submit', function(event) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ route('orders.transactions.store',$order->id)}}',
                        data: form_data,
                        success: function(response) {
