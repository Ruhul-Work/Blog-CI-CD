            ...
            <div class="row d-flex justify-content-center">
                <div class="col-md-8">
                    <div class="confirmation-page">
                        ...
                        <h1>অভিনন্দন!</h1>
                        <h2> আপনার সাবস্ক্রিপশন সফলভাবে সম্পন্ন হয়েছে</h2>
                        ...
                        <div>
                            {{-- <div class="order-summary">
                                <h2>অর্ডার ডিটেইলস</h2>
                                <div class="order-number">
                                    <strong>অর্ডার নম্বর:</strong> {{ $order->order_number }}
                                </div>
                                <div class="package-name">
                                    <strong>প্যাকেজ:</strong> {{ $order->package->name ?? 'প্যাকেজ পাওয়া যায়নি' }}
                                </div>
                                <div class="package-price">
                                    <strong>মূল্য:</strong> {{ number_format($order->package->current_price ?? 0, 2) }}৳
                                </div>
                                
                            </div> --}}
                            {{-- <table
                                style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border-bottom: 1px dashed #ddd;">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($order->orderItems as $item)
                                        <tr>
                                            <td style="padding: 8px;">{{ $loop->iteration }}.
                                                {{ $item->product->bangla_name }}</td>
                                            <td style="padding: 8px;">{{ $item->price }}</td>
                                            <td style="padding: 8px;">{{ $item->qty }}</td>
                                            <td style="padding: 8px; text-align: right;">{{ $item->price * $item->qty }}
                                            </td>
                                        </tr>
                                                ...
                                                <table style="width: 100%; border-collapse: collapse; margin-bottom: 10px;">
                                                    ...
                                                    @if ($order->packing_charge > 0)
                                                        <tr>
                                                            ...
                                                            <td style="padding: 8px; text-align: right;">
                                                                {{ formatPrice($order->packing_charge) }}</td>
                                                        </tr>
                                                    @endif
                                                    <tr>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="padding: 8px; text-align: right;">
                                                            <strong>{{ formatPrice($order->total, 2) }}</strong></td>
                                                    </tr>
                                                    ...
                                                    @if ($order->transactions->sum('amount') > 0)
                                                        <tr>
                                                            ...
                                                            <td style="padding: 8px; text-align: right;">
                                                                {{ formatPrice($order->transactions->sum('amount')) }}</td>
                                                        </tr>

                                                        <tr>
                                                            <td style="padding: 8px;"><strong>Due :</strong></td>
                                                            <td style="padding: 8px; text-align: right;">
                                                                <strong>{{ formatPrice($order->total - $order->transactions->sum('amount')) }}</strong>
                                                            </td>
                                                        </tr>
                                                    @endif
                                    ...
                                    <tr>
                                        <td colspan="4" style="padding: 8px;"><strong>Total in Words:</strong>
                                            {{ numberToWords($order->total) }}</td>
                                    </tr>
                                </tbody>
                            </table> --}}
