...
@section('meta')
    <title>All Blogs Tagged with {{ $tag->name }} | {{ get_option('title') }}</title>
        ...
        <div class="breadcrumb-outer">
            <div class="container">
                <div class="breadcrumb-content d-md-flex align-items-center pt-6">
                    <h2 class="mb-2">ট্যাগ ব্লগ</h2>
                    <nav aria-label="breadcrumb">
                        <ul class="breadcrumb mb-3 mt-5">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">হোম</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{ $tag->name }} ব্লগ</li>
                        </ul>
                    </nav>
                </div>
            </div>
                        ...
                        <div class="list-results d-flex align-items-center justify-content-between">
                            <div class="list-results-sort">
                                <p class="m-0">Showing {{ $blogs->firstItem() }}-{{ $blogs->lastItem() }} of
                                    {{ $blogs->total() }} results for <strong>{{ $tag->name }}</strong></p>
                            </div>
